...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        // updateAllAuthStatus will handle overlay visibility
        updateAllAuthStatus();
    }, 500);
});
...
function initAuthLogic() {
    ...
    function updateLoginStatus() {
        ...
        const downloadSectionOverlay = document.getElementById('downloadSectionOverlay');
        ...
        if (isLoggedIn && currentUser) {
            ...
            // Show download section and hide overlay (visible for all logged in users)
            if (downloadSection) {
                downloadSection.style.display = 'block';
                console.log('Download section displayed for logged in user');
            }
            if (downloadSectionOverlay) {
                downloadSectionOverlay.classList.add('hidden');
            }
            ...
            // Show download section with overlay (not logged in)
            if (downloadSection) {
                downloadSection.style.display = 'block';
            }
            if (downloadSectionOverlay) {
                downloadSectionOverlay.classList.remove('hidden');
            }
        }
    }
    ...
    function updateEnterpriseAuthStatus() {
        ...
        if (dataManagementSection) {
            // Always show the section, but control overlay visibility
            dataManagementSection.style.display = 'block';
            
            if (isLoggedIn && isEnterpriseAuthenticated) {
                // Hide overlay - user is authenticated
                if (dataManagementOverlay) {
                    dataManagementOverlay.classList.add('hidden');
                    console.log('Data management overlay hidden - user is authenticated');
                } else {
                    console.warn('Data management overlay element not found');
                }
                updateUserDataCount();
                console.log('Data management section displayed for enterprise authenticated user');
            } else {
                // Show overlay - user needs authentication
                if (dataManagementOverlay) {
                    dataManagementOverlay.classList.remove('hidden');
                    console.log('Data management overlay shown - user needs authentication. isLoggedIn:', isLoggedIn, 'isEnterpriseAuthenticated:', isEnterpriseAuthenticated);
                } else {
                    console.warn('Data management overlay element not found');
                }
            }
        ...
        // Update enterprise authentication status display in profile
        updateEnterpriseAuthStatusDisplay();
    }
    
    // Update enterprise authentication status display in profile page
    function updateEnterpriseAuthStatusDisplay() {
    ...
    if (enterpriseAuthForm) {
        enterpriseAuthForm.addEventListener('submit', function(e) {
            ...
            if (orgName && department) {
                ...
                // Update all authentication status (handles overlay visibility)
                updateAllAuthStatus();
                
                // Force hide overlay after a short delay to ensure DOM is ready
                setTimeout(() => {
                    const dataManagementOverlay = document.getElementById('dataManagementOverlay');
                    if (dataManagementOverlay && isLoggedIn && isEnterpriseAuthenticated) {
                        dataManagementOverlay.classList.add('hidden');
                        console.log('Force hide data management overlay after authentication');
                    }
                    updateAllAuthStatus();
                }, 100);
    ...
    if (isEnterpriseAuthenticated && enterpriseAuthData) {
        ...
        // updateAllAuthStatus already handles overlay visibility, just ensure count is updated
        if (isLoggedIn && isEnterpriseAuthenticated) {
            updateUserDataCount();
        }
    }
        ...
        insertForm.addEventListener('submit', function(e) {
            ...
            if (scientificName) {
                ...
                if (window.editingIndex !== undefined) {
                    ...
                    console.log('Total user data count:', userData.length);
...
function displayUserData() {
    ...
    if (userData.length === 0) {
        userDataList.innerHTML = '<div style="text-align: center; color: rgba(255,255,255,0.7); padding: 20px;">No data available</div>';
        return;
    }
...
function updateMapWithUserData() {
    ...
    console.log('Map updated, displaying user data points');
}
...
function initProfileSettings() {
    ...
    // Save email button event listener
    if (saveEmailBtn) {
        saveEmailBtn.addEventListener('click', function() {
            if (emailInput) {
                ...
                const originalText = saveEmailBtn.textContent;
                saveEmailBtn.textContent = 'Saved!';
                saveEmailBtn.style.background = '#45a049';
                setTimeout(() => {
                    saveEmailBtn.textContent = originalText;
